...
Open Settings → Slate to configure the plugin.

---

## Data folder

**Data folder path** — the vault folder where Slate stores task data. Default: `_slate_files`.
...
Assign a color to each label. If no color is set, Slate assigns a stable muted color automatically based on the label name.
...
**Enable Daily Notes integration** allows Slate to show completed tasks for the active daily note date.

**Daily note date format** controls how Slate matches a note path to a date. Default: `YYYY-MM-DD`.

**Auto-add completed tasks block** appends a `slate-completed` code block to daily notes that do not already have one. This is disabled by default.

Slate can show completed tasks in its own Daily Note panel or with a `slate-completed` Markdown code block. Auto-insert only writes the code block wrapper, not a static list of tasks.

---

## Fonts

Set the font used for different text areas inside Slate:
...
Slate data files are real vault files and may appear in Obsidian search and graph. To hide them, add your Slate data folder to:
