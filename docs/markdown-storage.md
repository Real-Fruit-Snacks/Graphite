...
Slate stores all task data as plain Markdown files inside your vault. There is no external database, no hidden file format, and no external service.
...
```
_slate_files/           ← configurable in Settings
├─ Data/
...
- [ ] Write portfolio case study draft
  ...
  attachments:: _slate_files/Attachments/abc123/sketch.png
```
...
If you do not want task data to appear in those surfaces, add your Slate data folder to Obsidian's excluded files list.

---

## Backups and sync

Slate task files are plain text and work with any file-based sync or backup system (iCloud, Obsidian Sync, Dropbox, git, etc.). Slate itself does not handle sync.
