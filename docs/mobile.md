...
Slate works on Obsidian Mobile (iOS and Android).

---

## Layout

On mobile, Slate uses a single-panel layout. The sidebar and task detail view stack vertically instead of showing side by side.
...
The **+ Add task** button is available at the top of every view. On mobile, Slate opens a full-screen composer so the title, description, metadata chips, project selector, and action buttons remain reachable around the keyboard.

You can also use `Slate: Add task` from the command palette.
